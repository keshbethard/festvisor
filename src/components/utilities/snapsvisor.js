export const data = [
    {
      id: "0001",
      name: "Helan går",
      melody: "Helan går",
      category: ['alla', 'populara', 'midsommar', 'jul', 'pask', 'nyar', 'kraftskiva'], //alla
      text: "Helan går<br/>Sjung hopp fallerallan lallan lej<br/>Helan går<br/>Sjung hopp fallerallan lej<br/>Och den som inte helan tar,<br/>Han heller inte Halvan får<br/>Helan går...<br/>(Helan tages)<br/>Sjung hopp faderallan lej.",
      sender: "anonym"
    },
    {
      id: "0002",
      name: "Till spritbolaget",
      melody: "Snickerboa",
      category: ['alla'],
      text: "Till spritbolaget ränner jag,<br/>Och bankar på dess port.<br/>Jag vill ha nå't som bränner bra,<br/>Och får mig skitfull fort.<br/><br/>Expediten sade goda',<br/> Hur gammal kan min herre va'.<br/>Har du nåt leg, ditt fula drägg,<br/>Kom hit igen när du har fått skägg.<br/><br/>Nej, detta var ju inte bra,<br/>Jag skall bli full ikväll.<br/>Då plötsligt en ide jag fick,<br/>De har ju sprit på Shell.<br/><br/>Många flaskor stod där på ra'.<br/>Så nu kan jag bli full och gla'.<br/>Den röda drycken åkte ner,<br/>Nu kan jag inte titta mer.",
      sender: "anonym"
    },
    {
      id: "0003",
      name: "Lille sup",
      melody: "Lille katt",
      category: ['alla', 'populara', 'midsommar', 'jul', 'pask', 'nyar', 'kraftskiva'], //alla
      text: "Lille sup, lille sup,<br/>Lille söte supen<br/>Nu ska du, nu ska du,<br/>Ner igenom strupen",
      sender: "anonym"
    },
    {
      id: "0004",
      name: "Älgen Hans",
      melody: "Helan går",
      category: ['alla'],
      text: "Älgen Hans, han äter både sten och grus<br/>Älgen Hans, han välter stora hus<br/>Älgen, älgen, älgen-älgen<br/>Älgen, älgen, älgen-älgen<br/>Älgen Haaaaaaaaaaaans...<br/>...han har så stora horn!",
      sender: "anonym"
    },
    {
      id: "0005",
      name: "Imse Vimsig",
      melody: "Imse Vimse spindel",
      category: ['alla'],
      text: "Imsig vimsig blir man av en liten hutt.<br/>Blodet börjar rusa, hjärtat tar ett skutt.<br/>Benen skälver, näsan den blir blå.<br/>Fast det är så läskigt<br/>      Vågar jag ändå.",
      sender: "anonym"
    },
    {
      id: "0006",
      name: "Min sup den har sin fyllning",
      melody: "Min hatt den har tre kanter",
      category: ['alla', 'populara', 'midsommar', 'jul', 'pask', 'nyar', 'kraftskiva'], //alla
      text: "Min sup den har sin fyllning,<br/>Sin fyllning har min sup.<br/>O har den ej nån fyllning,<br/>Så e de ej min sup.",
      sender: "anonym"
    },
    {
      id: "0007",
      name: "Lambo",
      melody: "Lambo",
      category: ['alla', 'midsommar'],
      text: "Alla: Sätt nu glaset till din mund!<br/>Tjofaderittan Lambo!<br/>Och drick ur, din fylle fylle hund!<br/>Tjofaderittan Lambo!<br/>Se hur dropparna i glaset<br/>Rinner ned i fylle aset.<br/>Lambo! Hej! Lambo! Hej!<br/>Tjofaderittan Lambo!<br/>Solo: Jag nu glaset druckit har.<br/>Alla: Tjofaderittan Lambo!<br/>Solo: Ej en droppe finnes kvar!<br/>Alla: Tjofaderittan Lambo!<br/>Solo: Som bevis jag nu vill vända,<br/>Glaset på dess rätta ända.<br/>Alla: Lambo hej! Lambo hej!<br/>Tjofaderittan Lambo!<br/>Ja, han kunde konsten,<br/>Han var en riktig fylle fylle hund.<br/>Låt oss gå till nästa man och se vad han förmår",
      sender: "anonym"
    },
    {
      id: "0008",
      name: "Festen i mitt hjärta",
      melody: "Stockholm i mitt hjärta",
      category: ['alla', 'kraftskiva'],      
      text: "Festen i mitt hjärta<br/>Låt oss besjunga en skål<br/>Här utpå våran veranda<br/>Vi dricker så mycket vi tål<br/>Med kräftor från hela världen<br/>Suger vi i oss allt<br/>Genom kräftornas kärlek till magen<br/>Får vi blandning av dill och salt",
      sender: "anonym"
    },
    {
      id: "0009",
      name: "Dom nubbarna",
      melody: "Små grodorna",
      category: ['alla', 'kraftskiva'],
      text: "Dom nubbarna, dom nubbarna<br/>Är lustiga att ha<br/>Dom nubbarna, dom nubbarna<br/>Dom vill vi gärna ha<br/>Ej röra, ej röra, nej skala kräftan först<br/>Och sedan, och sedan vi släcka ska vår törst.",
      sender: "anonym"
    },
    {
      id: "0010",
      name: "Blicken i skyn",
      melody: "Blicken i skyn",
      category: ['alla', 'populara', 'midsommar', 'jul', 'pask', 'nyar', 'kraftskiva'], //alla
      text: "Armen i vinkel,<br/>Blicken i skyn<br/>Så var det menat<br/>Whiskey och renat<br/>Vårt mål alkohol<br/>Skål för den som tål<br/>Skål!",
      sender: "Andreas S."
    },
    {
      id: "0011",
      name: "Lapin kulta",
      melody: "Broder Jakob",
      category: ['alla', 'finska'],
      text: "Lapin Kulta, Lapin Kulta,<br/>Karjala, Karjala<br/>Aura sekä Olvi<br/>Aura sekä Olvi<br/>Koff, Koff, Koff<br/>Koff, Koff, Koff",
      sender: "anonym"
    },
    {
      id: "0012",
      name: "Der er ingen bånd der binder mig",
      melody: "Der er ingen bånd der binder mig",
      category: ['alla', 'danska'],
      text: "Der er ingen bånd der binder mig<br/>Og mange øller drikker jeg<br/>Før var jeg en afholdsmand<br/>Og drak kun sodavand<br/><br/>Når vi til baller går mange mange øller får<br/>Vi er et alkohold<br/>Og vores øl skal være kold<br/><br/>Der er ingen bånd der binder mig<br/>Og mange øller drikker jeg<br/>Før var jeg en afholdsmand<br/>Og drak kun sodavand!<br/>",
      sender: "Agnete L."
    },
    {
      id: "0013",
      name: "Helan går",
      melody: "Helan går",
      category: ['alla', 'danska'],
      text: "Helan gå, hej kom faldera hurra hurra<br/>Helan gå, hej kom faldera hurra hurra<br/>Og den som ingen Helan går<br/>Han heller ingen Halan får<br/>Helan gå",
      sender: "Agnete L."
    },
    {
      id: "0014",
      name: "Kräfta, kräfta",
      melody: "Blinka lilla stjärna",
      category: ['alla', 'kraftskiva'],
      text: "Kräfta, kräfta prydd med dill,<br/>och en immig sup därtill,<br/>bröd och smör och ost och sill<br/>och så några supar till.<br/>Kräfta, kräfta prydd med dill,<br/>nu vi får allt vad vi vill",
      sender: "anonym"
    },
    {
      id: "0015",
      name: "Ta en sup",
      melody: "Tänd ett ljus",
      category: ['alla', 'kraftskiva'],
      text: "Ta en sup och låt det bränna<br/>låt aldrig halsen få känna, det är svalt nu<br/>men du blir varmare igen<br/>ta en sup för kräftan du käka<br/>akta mattan, sluta och kräka<br/>Ta en sup… för kräftans dag",
      sender: "anonym"
    },
    {
      id: "0016",
      name: "Kom alla till borden",
      melody: "Väva vadmal",
      category: ['alla', 'jul'],
      text: "Kom alla till borden<br/>i snöiga Norden.<br/>Här väntar maten<br/>och här är faten,<br/>nu är det jul uppå jord!<br/><br/>Stå inte och vänta,<br/>kom, pojke och jänta!<br/>Ja, kom på stubben<br/>här har du nubben,<br/>nu är det jul uppå jord!",
      sender: "anonym"
    },
    {
      id: "0017",
      name: "Midnatt snart",
      melody: "Midnatt råder, det är tyst i husen",
      category: ['alla', 'nyar'],
      text: "Midnatt råder<br/>snart om några timmar<br/>några timmar.<br/>Snapsen ner i<br/>torra halsen simmar<br/>snapsen simmar.<br/>Tick tack, tick tack,<br/>ticke ticke, tick tack<br/>tick, tick, Tack",
      sender: "anonym"
    },
    {
      id: "0017",
      name: "Sån är spriten",
      melody: "Sånt är livet",
      category: ['alla', 'populara', 'midsommar', 'jul', 'pask', 'nyar', 'kraftskiva'], //alla
      text: "Sån är spriten, ja sån är spriten<br/>Så mycket brännvin finns det här<br/>Den sup man ratar, den tar nån annan<br/>Så ta nu snapsen som du har kär.",
      sender: "anonym"
    },
    {
      id: "0018",
      name: "Julsnapsvisa",
      melody: "En sockerbagare",
      category: ['alla', 'jul'],
      text: "Den svenska julen har så många smaker<br/>som sill och skinka och pepparkaker.<br/>Och glöm sen inte all kryddad sprit.<br/>För svenska julen är inte vit!",
      sender: "anonym"
    },
    {
      id: "0019",
      name: "Tänk om jag hade lilla nubben",
      melody: "Hej tomtegubbar",
      category: ['alla', 'jul'],
      text: "Tänk om jag hade lilla nubben<br/>på ett snöre i halsen.<br/>Tänk om jag hade lilla nubben<br/>på ett snöre i halsen.<br/>Jag skulle dra den upp och ner,<br/>så att den kändes som många fl er.<br/>Tänk om jag hade lilla nubben<br/>på ett snöre i halsen.",
      sender: "anonym"
    },
    {
      id: "0020",
      name: "Vi tar ett rus",
      melody: "Nu har vi ljus",
      category: ['alla', 'jul'],
      text: "Vi tar ett rus<br/>här i vårt hus<br/>Helan är kommen,<br/>hoppfalleralla.<br/>Julen är här<br/>julgrisen är<br/>full som ett bär.<br/>Hej nu brinner all juleljusen<br/>tomten och Lucia är på snusen,<br/>Fallleralla, falleralla,<br/>Fallerallalleralla",
      sender: "anonym"
    },
    {
      id: "0021",
      name: "Här är vi ju alla vänner",
      melody: "We wish you a Merry X-mas",
      category: ['alla', 'jul'],
      text: "Här är vi ju alla vänner<br/>och alla varandra känner.<br/>Så skål på oss nu go vänner<br/>och välkomna hit.<br/>Vi skålar för våra vänner,<br/>och dom som vi känner.<br/>Men dom som vi inte känner,<br/>dom skiter vi i.",
      sender: "anonym"
    },
    {
      id: "0022",
      name: "Om man bara tar en smutt, smutt, smutt",
      melody: "Nu är glada julen slut, slut, slut",
      category: ['alla', 'jul'],
      text: "Om man bara tar en smutt, smutt, smutt<br/>Utav denna lilla hutt, hutt, hutt<br/>Har man ganska mycket kvar<br/>Men se den som spar han har<br/>Inte särskilt roligt",
      sender: "anonym"
    },
    {
      id: "0023",
      name: "Blinka lilla pärla där",
      melody: "Blinka lilla stjärna där",
      category: ['alla', 'jul'],
      text: "Blinka lilla pärla där<br/>hur jag undrar hur du är.<br/>Hoppas att du sitter bra<br/>så att jag blir riktigt gla ’.<br/>Blinka lilla pärla där,<br/>snart står du ej längre där.",
      sender: "anonym"
    },
    {
      id: "0024",
      name: "Är ni redo?",
      melody: "Hej tomtegubbar",
      category: ['alla', 'jul'],
      text: "Hej allesammans är ni redo<br/>för nu så börjar kalaset.<br/>Hej allesammans är det säkert<br/>att ni har fått nåt i glaset?<br/>En liten sup och julemat<br/>det är precis så som vi vill ha ’t.<br/>Höj allesammans högra armen<br/>Nu så tömmer vi glaset.",
      sender: "anonym"
    },
    {
      id: "0025",
      name: "Ser du stjärnan",
      melody: "When You Wish Upon a Star",
      category: ['alla', 'jul'],
      text: "Ser du stjärnan i det blå?<br/>Ta en sup så ser du två.<br/>Tar du mera, ser du flera<br/>stjärnor små.",
      sender: "anonym"
    }
  ];
  
  export default data;
